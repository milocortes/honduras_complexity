...
def _(pd, pl):
    ...
    def obten_datos(dataset : str) -> pd.DataFrame: 
        ...
        q = pl.scan_csv(f'datos/viabilidad_atractivo/{dataset}.csv').select("ACTIVITY", "OBS_VALUE", "MEASURE", "TIME_PERIOD").group_by("ACTIVITY", "TIME_PERIOD", "MEASURE").sum()
...
def _(pd):
    ## Carga FDI
    fdi = pd.read_csv("datos/viabilidad_atractivo/fDi_Subsectors/fdi_subsectores_iso_code3.csv")
